...
if ($_POST['mon_name'] != ""){
  ...
  $select = "SELECT mon_name ,mon_size , mon_type ,mon_hd ,mon_hp ,".
                   ...
                   "mon_environment, mon_level_adj, mon_magic_use, mon_delete, mon_ac_deflect, mon_ac_insight,  mon_ac_profane,  mon_ac_dodge, mon_ac_luck, mon_skill_rule " .
  ...
  $mon_ac_insight = $row['mon_ac_insight'];
  $mon_ac_profane = $row['mon_ac_profane'];
  $mon_ac_dodge = $row['mon_ac_dodge'];
  $mon_ac_luck = $row['mon_ac_luck'];
...
//  echo "</BR> " . $select . "</BR>";
 ...
 mysqli_free_result($result);
  $select = "SELECT monskillrb_tp, monskillrb_val, monskillrb_text, monskillrb_atr, monskillrb_classskill from monskillrb WHERE mon_name = '$mon_name'  and mon_key_1 = '$wp_user' ";
//  echo "</BR> " . $select . "</BR>";
 $result = mysqli_query($link, $select) ;
 $count = 0;
 while ($row = mysqli_fetch_array($result)) {
    ...
    $classskill_v = "monskillrb_classskill_" . $count;

    $$skillrb_v = $skillrb;
    $$rankrb_v = $rankrb;
    $$textrb_v = $textrb;
    $$atrrb_v = $atrrb;
    $$classskill_v = $classskill;
 }
 mysqli_free_result($result);
...
//  echo "result " $result;
 ...
 mysqli_free_result($result);
 $monbuff_level = "";
 $select = "SELECT monbuff_spell, monbuff_level from monbuff where mon_name = '$mon_name' and mon_key_1 = '$wp_user'";
 $count = 0;
 $result = mysqli_query($link, $select);
 while ($row = mysqli_fetch_array($result)) {
   $count +=1;
   $monbuff_spell_r = "monbuff_spell_".$count;
   $$monbuff_spell_r = $row['monbuff_spell'];
   $monbuff_level_x = $row['monbuff_level'];
   if ($monbuff_level_x !=""){
      $monbuff_level = $monbuff_level_x;
   }
 }
...
?>
<h2>Pathfinder RPG Change Monster</h2>
<?php
$SIZE = "" ;
if ($_POST) {
   $msg = "" ;
...
//       }
       ...
       if ($k == "mon_str" or $k == "mon_dex" or $k == "mon_con" or $k == "mon_int" or $k=="mon_wis" or $k=="mon_wis" or $k =="mon_chr"){
          if (is_numeric($$k)){
            if ($$k < 0 or $$k >60){
              $msg = "Monster Stats must be between 0 and 60";
              echo $k;
            }
          }else{
       ...
       if ($v == "" and $beg != "mon_fea" and $beg != "monskil" and $beg !="mon_spe"and $beg2 != "dam" and $beg2 != "spe"  
           and $beg !="mon_noc" and $k != "mon_ac_insight" and $k != "mon_ac_profane"  and $k != "mon_ac_luck"  and  $k != "mon_ac_dodge"
           and $k != "mon_template" and       $beg != "montrea" and $beg !="monorg_" and $beg !="monlang" and $beg != "monadv_" and $beg !="monbuff"){
...
//      include 'includes/dd_db_conn.txt';
      ...
      $insert  = "INSERT INTO monster2 (mon_key_1,mon_name ,mon_size , mon_type ,mon_hd ,mon_hp ,".
                   ...
                   "mon_cr ,mon_str ,mon_dex ,mon_con ,mon_int ," .
                   "mon_wis ,mon_chr ,mon_desc,mon_sv_fort, mon_sv_reflex,  mon_sv_will, " .
                   "mon_armour, mon_shield, mon_user, mon_nochange, mon_template, mon_alignment, mon_environment, mon_level_adj, " . 
                   "mon_magic_use, mon_delete, mon_ac_deflect, mon_ac_insight, mon_ac_profane, mon_ac_luck, mon_ac_dodge, mon_skill_rule) " .
           "VALUES ( '$wp_user','$mon_name' ,'$mon_size' , '$mon_type' ,'$mon_hd' ,'$mon_hp' ,".
                 "'$mon_init' ,'$mon_speed', '$mon_speed_fly', '$mon_speed_swim', '$mon_speed_burrow','$mon_speed_climb',".
                     ...
                     "'$mon_ac_deflect','$mon_ac_insight', '$mon_ac_profane','$mon_ac_luck','$mon_ac_dodge','$mon_skill_rule')";
...
//find the skill atribute and take off the skill value
           ...
           $classskill = "monskillrb_classskill_" . $count;
           if (!isset($$skill)){
                $$skill = "";
            }
            if (!isset($$value)){
           ...
           $monskillrb_atr = $$atr;
           $monskillrb_classskill =  $$classskill;
//         echo $value . "  " . $mon_skill_val;
           if ($monskillrb_tp != ""){
              $insert = "INSERT INTO monskillrb (mon_key_1, mon_name, monskillrb_tp , monskillrb_val, monskillrb_text, monskillrb_atr, monskillrb_classskill)".
                        "VALUES ('$wp_user','$mon_name', '$monskillrb_tp', '$monskillrb_val', '$monskillrb_text', '$monskillrb_atr', '$monskillrb_classskill')";
   //                        echo $insert;
              $result = mysqli_query($link, $insert);
              if (!$result) {
                 $msg = "$result" ." Error inserting skills";
...
//mon buffs
         ...
         While ($count <6){
           $count = $count  + 1;
           $monbuff_spell_v = "monbuff_spell_". $count;
           if (!isset($$monbuff_spell_v)){
                $$monbuff_spell_v = "";
           }
           $monbuff_spell = $$monbuff_spell_v;
           if ($monbuff_spell != ""){
             $insert = "INSERT INTO monbuff (mon_key_1, mon_name, monbuff_spell, monbuff_level)" .
                       " VALUES ('$wp_user','$mon_name','$monbuff_spell', '$monbuff_level')";
//              echo $insert;
           $result = mysqli_query($link, $insert);
               if (!$result) {
                  $msg = "$result" ." Error inserting monbuff";
               }
               else {
                  $msg = "record sucessfully added" ;
                  $fn = $ln = "" ;
...
?>
      <TD><SELECT NAME="mon_ac_insight" VALUE="<?php echo $mon_ac_insight ?>">
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
$select = "SELECT insight_key, insight_desc FROM insight " .
           "order by insight_key";
// echo "select" . $select;
  $result = mysqli_query($link, $select) ;
//  echo "result " $result;
  while ($row = mysqli_fetch_array($result)) {
    $insight_key_sel = $row['insight_key'] ;
    $insight_desc_sel = $row['insight_desc'] ;
    if ($insight_key_sel == $mon_ac_insight)
      {
       $sel = " SELECTED" ;
      } else {
       $sel = "" ;
...
echo <<<EOF
     <OPTION VALUE="$insight_key_sel" $sel > $insight_desc_sel </OPTION>
EOF;
   }
?>
</TR>
<TR>
    <TH>Profane AC Bonus</TH>
    <TH>Luck AC Bonus</TH>
    <TH>Dodge AC Bonus</TH>
</TR>
<TR>
      <TD><SELECT NAME="mon_ac_profane">
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
$select = "SELECT profane_key, profane_desc FROM profane " .
           "order by profane_key";
// echo "select" . $select;
  $result = mysqli_query($link, $select) ;
//  echo "result " $result;
while ($row = mysqli_fetch_array($result)) {
    $profane_key_sel = $row['profane_key'] ;
    $profane_desc_sel = $row['profane_desc'] ;
    if ($profane_key_sel == $mon_ac_profane){
       $sel = " SELECTED" ;
    }else{
       $sel = "" ;
    }
...
echo <<<EOF
     <OPTION VALUE="$profane_key_sel" $sel > $profane_desc_sel </OPTION>
EOF;
}
mysqli_free_result($result);
?>
      <TD><SELECT NAME="mon_ac_luck">
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
$select = "SELECT luck_key, luck_desc FROM luck " .
           "order by luck_key";
// echo "select" . $select;
  $result = mysqli_query($link, $select) ;
//  echo "result " $result;
while ($row = mysqli_fetch_array($result)) {
    $luck_key_sel = $row['luck_key'] ;
    $luck_desc_sel = $row['luck_desc'] ;
    if ($luck_key_sel == $mon_ac_luck){
       $sel = " SELECTED" ;
    }else{
       $sel = "" ;
    }
...
echo <<<EOF
     <OPTION VALUE="$luck_key_sel" $sel > $luck_desc_sel </OPTION>
EOF;
}
?>
      <TD><SELECT NAME="mon_ac_dodge">
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
$select = "SELECT dodge_key, dodge_desc FROM dodge " .
           "order by dodge_key";
// echo "select" . $select;
  $result = mysqli_query($link, $select) ;
//  echo "result " $result;
while ($row = mysqli_fetch_array($result)) {
    $dodge_key_sel = $row['dodge_key'] ;
    $dodge_desc_sel = $row['dodge_desc'] ;
    if ($dodge_key_sel == $mon_ac_dodge){
       $sel = " SELECTED" ;
    }else{
       $sel = "" ;
    }
...
echo <<<EOF
     <OPTION VALUE="$dodge_key_sel" $sel > $dodge_desc_sel </OPTION>
EOF;
}
mysqli_free_result($result);
mysqli_close($link);
...
<?php
$sel_half = $sel_1 = $sel_2 = $sel_10 = $sel_15 = $sel_20 = $sel_30 = $sel_35 = $sel_40 = $sel_45 = $sel_50 = "";
$var = "sel_" . $mon_space;
$$var = "SELECTED";
echo <<<EOF
     ...
     <OPTION VALUE="55" $sel_55 > 55 ft </OPTION>
     <OPTION VALUE="60" $sel_60 > 60 ft </OPTION>
     </SELECT>
     </TD>
EOF;
?>
...
<TR>
    <TH class="bigboxcc">Special Attacks</TH>
    <TH class="bigboxcc">Value</TH>
    <TH class="smallboxcc">Min HD</TH>
    <TH class="smallboxcc">Max HD</TH>
</TR>
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
...
while ($count < $count_max){
  $count = $count + 1;
   $specav = "speca_" . $count;
   $speca  = $$specav;
   if ($count == ($count_max -1) and $speca != ""){
      $count_max = $count_max + 10;
   }
   echo "<TR>";
echo <<<EOF
   <TD class="bigboxcc"><SELECT NAME='$specav'>
EOF;
...
echo <<<EOF
  <TD class="bigboxcc"><INPUT TYPE='text' NAME='$speca_valuev' VALUE='$speca_value'></TD>
  <TD class-"smallboxcc"><INPUT TYPE='text' NAME='$speca_minv' VALUE='$speca_min'></TD>
  <TD class="smallboxcc"><INPUT TYPE='text' NAME='$speca_maxv' VALUE='$speca_max'></TD>
EOF;
...
while ($count < $count_max){
  $count = $count + 1;
   $specqv = "specq_" . $count;
   $specq  = $$specqv;
   if ($count == ($count_max -1) and $specq != ""){
      $count_max = $count_max + 10;
   }
   echo "<TR>";
echo <<<EOF
...
//  $specq_value = str_replace("'", "", $specq_value);
//  str_replace("\'", "", $specq_value);
//  echo $specq_value;
...
?>
<BR></BR>
<H3>Constant Spells</H3>
<TABLE>
<TH>Caster Level:</TH><TD><INPUT TYPE='text' NAME='monbuff_level' VALUE='<?php echo $monbuff_level;?>'></TD>
</TABLE>
<TABLE>
<TR>
<?php
...
while ($count < 6){
   ...
   $buff_sel_v = "monbuff_spell_" . $count;
   $HTML .= '<TD><SELECT class="width40em" NAME="' . $buff_sel_v . '">';
   $buff_sel = $$buff_sel_v;
   if ($buff_sel == ''){
      $sel = " SELECTED";
   }else{
   ...
   $select = "select distinct spell.spell_name, spell_range from spellattr, spell, spellclass where spell.spell_name = spellclass_name and
            spell.spell_name = spellattr.spell_name and mon_key_1 = 'path' ";
   $result = mysqli_query($link, $select) ;
...
//    include 'includes/dd_db_conn.txt' ;
    ...
    $mon_skill_val = $$value;
    $mon_skill = $$name;
    if ($count1 == ($count_max -1) and $mon_skill != ""){
      $count_max = $count_max + 10;
     }
...
//    include 'includes/dd_db_conn.txt' ;
   ...
   $classskill = 'monskillrb_classskill_' . $sub;
   $monskill_val = $$value;
   $monskillrb = $$name;
   $monskill_text = $$text;
   $monskill_atr = $$atr;
   $monskill_classskill = $$classskill;
...
echo <<<EOF
    <TD><INPUT TYPE="text" NAME="$text" VALUE="$monskill_text"/></TD>
    <TD><INPUT TYPE="text" NAME="$value" VALUE="$monskill_val"/></TD>
    <TD><INPUT TYPE="text" NAME="$atr" VALUE="$monskill_atr"/></TD>
    <TD><INPUT TYPE="text" NAME="$classskill" VALUE="$monskill_classskill"/></TD>
    </TR>
EOF;
}
//mysqli_free_result($result);
...
<TR>
<TH id="bigbox2">Monster Feats</TH>
<TH  id="smallbox2">Free?</TH>
<TH id="bigbox2">Monster Feats</TH>
<TH id="smallbox2">Free?</TH>
</TR>
...
While ($count < $count_max) {
  ...
  $name = "mon_feat_" . $count;
  $mon_feat = $$name;
  if ($count == ($count_max -1) and $mon_feat != ""){
      $count_max = $count_max + 10;
  }
  $free_v = "mon_feat_free_" . $count;
  $free = $$free_v;
...
echo <<<EOF
    <TD id="bigbox2"><SELECT NAME="$name">
EOF;
...
echo <<<EOF
<TD id="smallbox2"><SELECT NAME="$free_v">
...
echo <<<EOF
     ...
     <OPTION VALUE="AC" $sel_AC > Animal Companion</OPTION>
