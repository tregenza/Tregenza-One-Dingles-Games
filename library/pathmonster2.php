...
if ($_POST) {
   $msg = "" ;
    foreach ($_POST  as $k => $v) {
       ...
       if ($v == "" and $beg != "mon_fea" and $beg != "monskil" and $beg !="mon_spe"and $beg2 != "dam" and $beg2 != "spe" and $k != "mon_template"
            and $beg !="monlang" and $beg != "monorg_" and $beg != "montrea"  and $beg != "monadv_" and $k != "mon_ac_insight" and $k != "mon_ac_profane"){
          $msg = "please fill in all fields";
           echo $k;
       }

   }
   $select = "Select count(*) from monster2 where mon_name = '$mon_name' and (mon_key_1 = 'path' or mon_key_1 = '$wp_user')";
   $link = getDBLink();
   ...
   if ($mon_type !=""){
     $select = "select montype_hd, montype_skillp from montype2 where montype = '$mon_type' and mon_key_1 = 'path'";
     $result = mysqli_query($link, $select);
     $row = mysqli_fetch_array($result);
  ...
  //    echo $atr_pluss . ":" . $montype_skillp . ":" . $mon_hitdie;
   ...
   if ($msg == "") {
      ...
      $insert  = "INSERT INTO monster2 (mon_key_1, mon_name ,mon_size , mon_type ,mon_hd ,mon_hp ,".
                   ...
                   "mon_magic_use, mon_delete, mon_ac_deflect, mon_ac_insight, mon_ac_profane, mon_skill_rule) " .
           "VALUES ( '$mon_key_1','$mon_name' ,'$mon_size' , '$mon_type' ,'$mon_hd' ,'$mon_hp' ,".
                 "'$mon_init' ,'$mon_speed', '$mon_speed_fly', '$mon_speed_swim', '$mon_speed_burrow','$mon_speed_climb',".
                     ...
                     "'$mon_alignment','$mon_environment', '$mon_level_adj', '$mon_magic_use', '$mon_delete','$mon_ac_deflect','$mon_ac_insight','$mon_ac_profane','$mon_skill_rule')";
...
//  find if any feats have been included for the skill
              $select = "select featattr_no from featattr2, monfeat2 where ".
                        ...
                        "featattr_id = '$mon_skill' and featattr_feat = monfeat and featattr2.mon_key_1 = 'path' and monfeat2.mon_key_1 = 'path'";
              $result = mysqli_query($link, $select);
              if ($result){
                  $row = mysqli_fetch_array($result);
...
//              echo "</BR> armour_check $armout_check";
              ...
              $select = "SELECT skill_cd from montpskill where mon_key_1 = 'path' and montype =  '$mon_type' and skill_cd = '$mon_skill'";
              $result_type = mysqli_query($link, $select);
//add in skill pluss for mon type only if skill ranks exist.
...
if (!isset($mon_environment)){
   $mon_environment = "";
}
if (!isset($mon_ac_deflect)){
   $mon_ac_deflect = "";
}
if (!isset($mon_ac_insight)){
   $mon_ac_insight = "";
}
if (!isset($mon_ac_profane)){
   $mon_ac_profane = "";
}
...
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
$select = "select montype from montype2 where mon_key_1 = 'path' ".
           "order by montype";
// echo $select;
   $result = mysqli_query($link, $select) ;
//  echo "result " $result;
...
<?php
$sel_T = $sel_0 = $sel_L = $sel_AC = $sel_N = "";
...
<TR>
<TH>Alignment</TH>
<TD><INPUT TYPE="text" NAME="mon_alignment" VALUE="<?php echo $mon_alignment ?>"></TD>
<TH>Level Adjustment</TH>
<TD><INPUT TYPE="text" NAME="mon_level_adj" VALUE="<?php echo $mon_level_adj?>"></TD>
</TR>
</TABLE>
<BR></BR>
<BR></BR>
...
?>
      <TD><SELECT NAME="mon_ac_profane" VALUE="<?php echo $mon_ac_profane ?>">
<?php
$link = getDBLink();
//include 'includes/dd_db_conn.txt' ;
