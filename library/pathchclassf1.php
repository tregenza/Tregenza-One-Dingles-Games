...
while ($loop < 1){
  $loop = $loop + 1;
  $select = "SELECT class_tp from class2 where mon_key_1 = 'path' ORDER BY class_tp";
  ...
  $result = mysqli_query($link, $select) ;
   while ($row = mysqli_fetch_array($result)) {
     ...
     if ($type != ""){
       ...
       $select2 = "SELECT DISTINCT classf_focus from classfocus2 where classf_class = '$type' and mon_key_1 = 'path' ORDER BY classf_class";
//  echo $select . "</BR>";
       include 'includes/dd_db_conn.txt';
...
if ($local == "paulds-1.vm.bytemark.co.uk"){
   $location = '"http://' . $local . '/apache2-default/pathchclassf2.php"';
}else{
   $location =  '"pathchclassf2.php"';
}
//echo $location;
if ($msg != ""){
  echo "<div class=\"error\">$msg</div>" ;
