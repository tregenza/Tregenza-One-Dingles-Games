...
//   echo "class" . $class .  $class_v . " key_1 $key_1";
 ...
 //  echo "spell_level ". $spell_level;
  ...
  //      echo "max spell level $max_spell_level";
   }
...
function printSpells($classNumber){   
  ...
  global $$name_v;
  $name = $$name_v;
//  echo "<div>name $name name_v $name_v</div>";
...
*/
 ...
 // echo "class_spell_level $class_spell_level";
  ...
  $stat   = $$stat_v;
 // echo "spat = $spat classNunber = $classNumber"; 
  $stat_b = magicstat($spat);
  $stat += $stat_b;
  $max_level = $stat - 10;
  while ($spell_level < 10){
...
//   echo "magic key 1 = $key_1";
 ...
 //      echo $body;
       ...
       while ($number < $max){
          ...
          global $$magic_item_v;
          $magic_item = $$magic_item_v;
       //   $char = substr($magic_item,8,1);
     //     echo "$char = $char ord =" . ORD($CHAR);
      //    echo "<P>" . $magic_item_v . " = "  . $magic_item . "</P>";
          if ($magic_item != ""){
          //   $char = substr($magic_item,8,3);
   //          declare(encoding='UTF-8');
      //       $str = $char;
     //        for ( $pos=0; $pos < strlen($str); $pos ++ ) {
        //         $byte = substr($str, $pos);
       //           echo 'Byte ' . $pos ." " . $byte . " of " . $str . ' has value ' . ord($byte) . PHP_EOL;
       //      }
        //     $char = chr(172);
        //     echo "<p> before $magic_item </p>";
        //     $magic_item = strtr($magic_item,$char,"+");
        //     echo "<p>after $magic_item</p>";
   //          echo "<p> chr = " . chr(172) . "</p>";
   //          echo "$char = $char ord =" . ORD($CHAR);
             $magic_found = "Y";
            ...
            //     echo "<p>desc $magic_desc </p>";
                ...
                //   $magic_item = "";
                 ...
                 if ($magic_price_bonus > 0){
                   ...
                   $html .=  $magic_item . " " . $magic_desc . " (" . $magic_value . " bonus) </BR></div><div>";
                   $html_s .=  $magic_item . " " . $magic_desc . ", ";
                 }else{
                   $print .= "\n" . $magic_item . " " . $magic_desc . " (" . $magic_value . "gp)";
                   $html .=  $magic_item . " " . $magic_desc . " (" . $magic_value . "gp) </BR></div><div>";
                   $html_s .=  $magic_item . " " . $magic_desc . ", ";
                 }
              }
          }
   ...
   $html .=  "</BR></BR></div><div></div><div><b> Total Value = " . $total_value . "</b>";
...
//  echo "pat int = " . $_POST['mon_int'];
  ...
  if (isset($_POST['mon_int'])){
       $mon_int_post =   $_POST['mon_int'];
  }
  if ($mon_int == 0 and  ($mon_int_post == "" or $mon_int_post == 0)){
...
function getSaveMon($savemon_monster){
  ...
  //    $savemon_monster = strtr($savemon_monster,$char,"+");
      parse_str($savemon_monster,$monster_a);
//      echo "getSaveMon" ;
...
//      $monster_a = array('$savemon_monster');
 //     echo $savemon_monster;
      foreach ($monster_a as $k => $v) {
        ...
        $v = str_replace("|", "'", $v);
        $v = str_replace("#", "&", $v);
//      This replaces a duff + with a real + so it comes out correctly in the print. 16/08/18
        $char = chr(172);
        $v = strtr($v,$char,"+");
...
function buff_attr(){
  ...
  while ($buff_no < 10){
    ...
    if ($buff_spell != ""){
      ...
      $htmlp_buff .= $$buff_spell_v . " caster level " . $$buff_level_v . "</BR></div><div>";
      $magic_found = "Y";
...
function printattacks(){
 ...
 //   echo $select;
   ...
   while ($row = mysqli_fetch_array($result)) {
       ...
       $attacktemp_count = $row['attacktemp_count'];
       $attacktemp_prim = $row['attacktemp_prim'];
   //    echo "attack desc =  $attacktemp_desc";
       if  (strpos($attacktemp_desc, "No Melee")){
        $mult = "";
         ...
         if ($print_attack <> "" and $count == 1){
            $htmlp_secondary_attacks .= "(" . $print_attack . ")</BR></div><div>";
            $htmlp_secondary_attacks_s .= "(" . $print_attack . ")";
         }else{
            $htmlp_secondary_attacks  .="</BR></div><div>";
...
Function check_domain_remove($class,$level,$spec){
  global $class1_tp, $class1_level, $class2_tp, $class3_tp, $class2_level, $class3_level, $class1_feat;
  global $domain_11, $domain_12, $domain_13, $domain_21, $domain_22, $domain_23, $domain_31, $domain_32, $domain_33;
  if ($class == $class1_tp){
     $domain = $domain11;
  }else{
...
Function check_specials_remove($spec, $no){
  global $class1_tp, $class1_level, $class2_tp, $class3_tp, $class2_level, $class3_level, $class1_feat, $key_1;
  global $domain_11, $domain_12, $domain_13, $domain_21, $domain_22, $domain_23, $domain_31, $domain_32, $domain_33;
  $sel1 = "";
  $sel2 = "";
...
//echo "spec level " . $spec_level;
  while ($row = mysqli_fetch_array($result)) {
   ...
   //   echo "<div>monspec name $monspec_name </div>";
...
//        echo "$domain_v $domain_m1";
  ...
  //  echo "type = :" . $specatta_type .":";
      if ($specatta_type == "AURA"){
        ...
        if ($specatta_type == "SPELL"){
          $htmlp_spell_abil .=  "<div>" . $row['monspec_name'] . "$DC_txt" . $row['monspec_value']. "</BR></div>";
...
function OLD_add_BR ($html){
   ...
   $html = str_replace("</BR>","</BR></div><div>",$html);
   return($html);
}
