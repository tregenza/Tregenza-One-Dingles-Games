...
if ($_POST) {
  ...
  if ($msg == "") {  
    ...
    while ($loop < 9){ 
      ...
      $featattr_levelv = "featattr_level_" . $loop;
      $featattr_level  = $$featattr_levelv;
      if ( $featattr_level  == ""){
         $featattr_level  = 0;
      }   

      if ($featattr_type !== "") {
        $insert = "INSERT into featattr2
                 (featattr_feat, featattr_type, featattr_id, featattr_no, featattr_rfeat,featattr_level, mon_key_1)
                 VALUES ('$feat','$featattr_type','$featattr_id', '$featattr_value','$featattr_rfeat',$featattr_level,'path')" ;
//        echo $insert;
...
echo <<<EOF
  ...
  <TD><INPUT TYPE="text" NAME="$featattr_levelv" VALUE= $featattr_level ></TD>
EOF;
echo "</TR>" ;
echo   "</BR>";
}
